...
function setlevel () {
    ...
    if (level == 1) {
        ...
        scene.setTileMap(img`
            ...
            2.......................7241.....aaaaaab2.7aaaa2.2.2.2........2..2....
            2222222222222aa22222222222222222222aaa222222222212.2.2..2.......22....
...
function on_start () {
    jump_speed = 125
    game.splash("it is impossible to lose", "(press 'A')")
    if (game.ask("Would you like to do...", "training?")) {
        ...
        scene.setTileMap(img`
            ...
            `)
        if (instructions_on) {
            game.splash("Use the left and right buttons to move to the star", "(press 'A' to dismiss)")
        }
...
function instrunctions () {
    if (level == 1) {
        ...
        game.splash("Now practice jumping out of a tight spot")
    } else if (level == 8) {
    ...
    if (star == 0) {
        ...
        if (key) {
            scene.setTile(7, img`
                ...
                . . . . . . . . . . . . . . . . 
                `, false)
            tiles.placeOnTile(dog.sprite, tiles.getTileLocation(26, 13))
            game.setDialogFrame(img`
                .....cccccccccccccc.....
                ...cbd111111111111dbc...
...
scene.onHitTile(SpriteKind.Player, 2, function (sprite) {
    if (in_jump && dog.jumpVelocity > jump_speed && dog.sprite.vy >= 50) {
        dog.jumpVelocity = jump_speed
        myDigit.setDigitColor(0)
        in_jump = false
    }
...
game.showLongText("press 'A' (space if you have a keyboard) again to begin your adventure", DialogLayout.Bottom)
pause(100)
scene.setBackgroundImage(img`
    
...
game.onUpdateInterval(500, function () {
    if (enable_traveling_block_1) {
        scene.setTileAt(scene.getTile(col, row), 0)
        col += 1
        if (max + sc < col) {
            col = sc
        }
        ...
        if (enable_traveling_block_2) {
            scene.setTileAt(scene.getTile(col2, row2), 0)
            col2 += 1
            if (max2 + sc2 < col2) {
                col2 = sc2
            }
